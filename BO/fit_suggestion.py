...
def suggest_from_CP(data_by_ctype, idxsInput_by_ctype, bi_by_ctype, fix_by_input=None, range_by_input=None):
    ...
    for d_x, bi in zip(data_by_ctype, bi_by_ctype):
        ...
        if bi:
            ...
            # print(train_x.shape, labels_x.shape)
            pass
        dim_by_ctype.append(train_x.shape[1])
    ...
    vals_by_var, idxsVar_by_ctype, dimVar_by_input, idxInput_to_idxsVar, fix_by_var = inputs_to_vars(vals_by_input,
                                                                                                     idxsInput_by_ctype,
                                                                                                     fix_by_input)
    for idxsVar, dim in zip(idxsVar_by_ctype, dim_by_ctype):
        assert len(idxsVar) == dim, f'idxsVar_by_ctype:{idxsVar_by_ctype} vs dim_by_ctype:{dim_by_ctype}'
    ...
    class TempConstraintProblem(Problem):
        ...
        def __init__(self, n_var=0, n_obj=0, n_constr=0, xl=[], xu=[], use_ent=True):
        ...
        def _evaluate0(self, X, out, *args, **kwargs):
            ...
            for idxs, mll_model, bi, test, inputDim in zip(idxsVar_by_ctype, mll_model_by_ctype, bi_by_ctype,
                                                           test_by_ctype, dim_by_ctype):
                mll, model = mll_model
                xg = var[:, idxs]
                if xg.ndim <= 1:
                    xg = xg.unsqueeze(1)
                assert xg.shape[
                           1] == inputDim, f"Input dimension for GP is {inputDim} but you tried feeding a tensor of size {xg.shape[1]} with idx's {idxs}"
