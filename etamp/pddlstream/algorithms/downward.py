...
from ..language.constants import EQ, NOT, Head, Evaluation, get_prefix, get_args, OBJECT, TOTAL_COST, pAction
from ..language.conversion import is_positive_evals, is_negated_eval, objects_from_evaluations, \
    pddl_from_object, \
    pddl_list_from_expression, pddl2obj
from ..utils import read, write, INF, clear_dir, get_file_path, MockSet, find_unique, int_ceil

filepath = os.path.abspath(__file__)
if ' ' in filepath:
    raise RuntimeError('The path to pddlstream cannot include spaces')

FD_PATH = get_file_path(__file__, '../../../symk/')
USE_CERBERUS = False


def find_build(fd_path):
    for release in ['release64', 'release32', 'release']:  # TODO: list the directory
...
def parse_domain_pddl(*args):
    return pddl_parser.parsing_functions.parse_domain_pddl(*args)
...
for w in range(1, 1 + 5):
    SEARCH_OPTIONS['ff-wastar{}'.format(w)] = '--heuristic "h=ff(transform=adapt_costs(cost_type=NORMAL))" ' \
                                              ...
                                              'preferred_successors_first=true,cost_type=NORMAL,max_time=%s,bound=%s)"'.format(
        w)
...
def parse_sequential_domain(domain_pddl, action_info={}):
    if isinstance(domain_pddl, Domain):
        return domain_pddl
    iter_domain_pddl = parse_domain_pddl(parse_lisp(domain_pddl), action_info)
...
def fact_from_fd(fd):
    ...
    return (fd.predicate,) + tuple(map(pddl2obj, fd.args))


def evaluation_from_fd(fd):
    if isinstance(fd, pddl.Literal):
        head = Head(fd.predicate, tuple(map(pddl2obj, fd.args)))
...
def fd_from_evaluation(evaluation):
    """
    Generate Atom/NegatedAtom/Assign out of the evaluation
    """
    name = evaluation.head.function
    args = tuple(map(pddl_from_object, evaluation.head.args))
    if is_positive_evals(evaluation):
        return pddl.Atom(name, args)
    elif is_negated_eval(evaluation):
...
def get_problem(optms_evaluations, goal_expression, domain, unit_costs=False):
    objects = objects_from_evaluations(optms_evaluations)
    typed_objects = list({pddl.TypedObject(pddl_from_object(obj), OBJECT) for obj in objects} - set(domain.constants))
    """Get Atom/NegatedAtom/Assign from evaluations.
       the objects in args are turned into strings here."""
    optms_atoms = [fd_from_evaluation(e) for e in optms_evaluations
    ...
    return Problem(task_name=domain.name, task_domain_name=domain.name,
                   ...
                   init=optms_atoms, goal=goal, use_metric=not unit_costs)


def get_name2axioms(axioms):
    dict_name_axioms = defaultdict(list)
    for axiom in axioms:
        dict_name_axioms[axiom.name].append(axiom)
    return dict_name_axioms


def literal_holds(state, literal):
    # literal: is Atom or NegatedAtom
    if literal.negated:
        return not (literal.negate() in state)
    else:
        return literal in state


def condition_hold(state, condition):
    result = all(literal_holds(state, literal)
                 for literal in condition)
    """will be True is conditions = []"""
    return result
...
def get_conditional_effects(operator):
    if isinstance(operator, pddl.PropositionalAction):
        del_c_effs = [(cond, effect.negate())
                      for cond, effect in operator.del_effects]
        add_c_effs = [(cond, effect)
                      for cond, effect in operator.add_effects]
        return add_c_effs + del_c_effs
...
def is_applicable(state, action):
    return condition_hold(state, get_precondition(action))


def apply_propo_action(init_facts, action):
    """
    Utilize the input propositional action to expand the init_facts,
    if the action is applicable.
    """
    assert (isinstance(action, pddl.PropositionalAction))

    for eff_condition, effect in action.del_effects:
        if condition_hold(init_facts, eff_condition):
            init_facts.discard(effect)
    for eff_condition, effect in action.add_effects:
        if condition_hold(init_facts, eff_condition):
            init_facts.add(effect)
...
##################################################
def get_preimage_literals_with_idxes(stepaa_plan, goal=[]):
    """
    Return all preimage literals of a stepaa/preimage plan, with step indexes where they arise.

    Preimage literals of an operator:= precondition literals - effect literals.
    The step indexes of a literal: indicate in which steps it appears.
    """
    propo_actions = [item for item in stepaa_plan
                     if isinstance(item, pddl.PropositionalAction)]

    step_idx = len(propo_actions)
    # The goal literals have the maximum step index
    literal_to_idxes = {literal: {step_idx}
                        for literal in goal}

    """Reversely traverse the plan: from end to start."""
    for operator in reversed(stepaa_plan):  # reverse()返回一个反转序列的迭代器
        if isinstance(operator, pddl.PropositionalAction):
            """The step index depends on the positions of actions."""
            step_idx -= 1
            action_preimage(operator, literal_to_idxes, step_idx)
        elif isinstance(operator, pddl.PropositionalAxiom):
            axiom_preimage(operator, literal_to_idxes, step_idx)
        else:
            raise ValueError(operator)
    return literal_to_idxes
...
def make_preconditions_disconj(preconditions):
    return pddl.Disjunction(list(map(fd_from_fact, preconditions)))
...
def make_conditional_effect_conj(cond_facts, eff_fact):
    return pddl.Effect(parameters=[], condition=make_preconditions(cond_facts),
                       literal=fd_from_fact(eff_fact))


def make_conditional_effect_disconj(cond_facts, eff_fact):
    return pddl.Effect(parameters=[], condition=make_preconditions_disconj(cond_facts),
                       literal=fd_from_fact(eff_fact))


def make_conditional_effect(conditions, eff_fact):
    return pddl.Effect(parameters=[], condition=pddl.Conjunction(conditions),
                       literal=fd_from_fact(eff_fact))
