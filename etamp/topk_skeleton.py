...
def build_nt_problem(nt_domain, _init, _goal):
    ...
    if _goal.parts:
        ...
        goal.add(tuple([_goal.predicate] + list(_goal.args)))
...
def apply_streams_domain(nt_domain, optms_results, stream_file):
    ...
    for block in stream_blocks:
        if block[0] == ':stream':
            ...
            assert len(stream.inputs) == len(stream.input_type_list) * 3, f"{stream.name} misses input_type."
            assert len(stream.outputs) == len(stream.output_type_list) * 3, f"{stream.name} misses output_type."
...
def sk_from_file(path):
    ...
    for a in actions:
        ...
        for arg in args:
            if arg[0] == '#':
                if arg not in dict_optms_obj:
                    dict_optms_obj[arg] = '#' + str(len(dict_optms_obj) + 1)
                sk_args.append(dict_optms_obj[arg])
            else:
                sk_args.append(arg)
...
class TopkSkeleton(object):
    ...
    def __init__(self, original_domain_file, stream_file, original_init, goal,
        ...
        self.ap_files = []
        num_raw_target = round(target_num * 1.5)
        ...
        for i in range(50):
            ...
            if len(self.ap_files) >= target_num:
                ...
                print(f'{len(self.ap_files)}/{target_num} action_plans are found.')
                num_raw_target = round(num_raw_target * 1.5)
